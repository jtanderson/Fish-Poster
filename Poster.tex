        ...
        \begin{minipage}[T]{.95\textwidth} % tweaks the width, makes a new \textwidth
          \parbox[t][\columnheight]{\textwidth}{ % must be some better way to set the the height, width and textwidth simultaneously
            ...
            \begin{block}{Deep Belief Network}
                \begin{figure}[htbp]
                   \centering
                   \subfloat[RBM Architecture]{\includegraphics[height=5\baselineskip]{rbmarch}} % requires the graphicx package
                   \quad
                   \subfloat[DBN Architecture]{\includegraphics[height=5\baselineskip]{dbnarch}}
                   \label{fig:}
                \end{figure}
              \begin{itemize}
